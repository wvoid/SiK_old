...
;--------------------------------------------------------
	.area	OSEG    (OVR,DATA)
_mavlink_crc_length_1_151:
	.ds 1
_mavlink_crc_tmp_2_152:
	.ds 1
;--------------------------------------------------------
...
;------------------------------------------------------------
;sum                       Allocated with name '_mavlink_crc_sum_1_151'
;i                         Allocated with name '_mavlink_crc_i_1_151'
;stoplen                   Allocated with name '_mavlink_crc_stoplen_1_151'
;crc_extra                 Allocated to registers r7 
;length                    Allocated with name '_mavlink_crc_length_1_151'
;tmp                       Allocated with name '_mavlink_crc_tmp_2_152'
;------------------------------------------------------------
...
;	radio/mavlink.c:57: register uint8_t length = pbuf[1];
	...
	movx	a,@dptr
	mov	_mavlink_crc_length_1_151,a
;	radio/mavlink.c:58: __xdata uint16_t sum = 0xFFFF;
	mov	dptr,#_mavlink_crc_sum_1_151
	mov	a,#0xFF
	movx	@dptr,a
	inc	dptr
	movx	@dptr,a
;	radio/mavlink.c:61: stoplen = length + 6;
	mov	a,#0x06
	add	a,_mavlink_crc_length_1_151
...
;	radio/mavlink.c:71: tmp ^= (tmp<<4);
	mov	_mavlink_crc_tmp_2_152,a
	swap	a
	anl	a,#0xF0
	xrl	_mavlink_crc_tmp_2_152,a
;	radio/mavlink.c:72: sum = (sum>>8) ^ (tmp<<8) ^ (tmp<<3) ^ (tmp>>4);
	mov	ar2,r3
	mov	r3,#0x00
	mov	r0,_mavlink_crc_tmp_2_152
	mov	r1,#0x00
	mov	ar6,r0
	clr	a
	xrl	ar2,a
...
;------------------------------------------------------------
;m                         Allocated with name '_MAVLink_report_m_1_154'
;------------------------------------------------------------
