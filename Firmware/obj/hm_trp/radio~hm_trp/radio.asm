...
_preamble_detected:
	.ds 1
_radio_preamble_detected_EX0_saved_1_154:
	.ds 1
_radio_transmit_simple_transmit_started_1_167:
	.ds 1
_radio_transmit_ret_1_183:
	.ds 1
_radio_transmit_EX0_saved_1_183:
	.ds 1
...
_scale_uint32_PARM_2:
	.ds 4
_scale_uint32_value_1_238:
	.ds 4
_set_frequency_registers_frequency_1_246:
	.ds 4
;--------------------------------------------------------
; external ram data
;--------------------------------------------------------
...
_radio_buffer::
	.ds 252
_radio_receive_packet_length_1_140:
	.ds 3
_radio_transmit_length_1_182:
	.ds 1
_radio_set_channel_channel_1_196:
	.ds 1
_radio_set_transmit_power_power_1_220:
	.ds 1
_radio_set_transmit_power_i_1_221:
	.ds 1
_radio_set_network_id_id_1_226:
	.ds 2
_set_frequency_registers_band_1_247:
	.ds 1
_radio_set_diversity_state_1_252:
	.ds 1
;--------------------------------------------------------
...
;------------------------------------------------------------
;length                    Allocated with name '_radio_receive_packet_length_1_140'
;------------------------------------------------------------
...
_radio_receive_packet:
	...
	mov	r6,dph
	mov	a,dpl
	mov	dptr,#_radio_receive_packet_length_1_140
	movx	@dptr,a
	mov	a,r6
	inc	dptr
	movx	@dptr,a
...
;	radio/radio.c:243: EX0_SAVE_DISABLE;
	mov	c,_EX0
	mov	_radio_preamble_detected_EX0_saved_1_154,c
	clr	_EX0
...
;	radio/radio.c:246: EX0_RESTORE;
	mov	c,_radio_preamble_detected_EX0_saved_1_154
	mov	_EX0,c
...
;	radio/radio.c:339: transmit_started = false;
	clr	_radio_transmit_simple_transmit_started_1_167
...
;	radio/radio.c:354: if (transmit_started && length != 0 && (status & EZRADIOPRO_ITXFFAEM)) {
	jnb	_radio_transmit_simple_transmit_started_1_167,00108$
	mov	a,r7
	jz	00108$
	mov	a,r3
	jnb	acc.5,00108$
...
;	radio/radio.c:370: if (transmit_started && length != 0 && (status & EZRADIOPRO_ITXFFAFULL) == 0) {
	jnb	_radio_transmit_simple_transmit_started_1_167,00114$
	mov	a,r7
	jz	00114$
	mov	a,r3
	jb	acc.6,00114$
...
;	radio/radio.c:402: transmit_started = true;
	setb	_radio_transmit_simple_transmit_started_1_167
;	radio/radio.c:403: continue;
	ljmp	00130$
00122$:
;	radio/radio.c:405: if (transmit_started && (status & 0x02) == 0) {
	jb	_radio_transmit_simple_transmit_started_1_167,00214$
	ljmp	00130$
00214$:
	mov	a,r3
	jnb	acc.1,00215$
...
;------------------------------------------------------------
;length                    Allocated with name '_radio_transmit_length_1_182'
;------------------------------------------------------------
...
_radio_transmit:
	mov	a,dpl
	mov	dptr,#_radio_transmit_length_1_182
	movx	@dptr,a
;	radio/radio.c:507: EX0_SAVE_DISABLE;
	mov	c,_EX0
	mov	_radio_transmit_EX0_saved_1_183,c
	clr	_EX0
;	radio/radio.c:520: ret = radio_transmit_simple(length, buf, timeout_ticks);
	...
	mov	_radio_transmit_ret_1_183,c
;	radio/radio.c:526: EX0_RESTORE;
	mov	c,_radio_transmit_EX0_saved_1_183
	mov	_EX0,c
;	radio/radio.c:527: return ret;
	mov	c,_radio_transmit_ret_1_183
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;status                    Allocated with name '_radio_initialise_status_1_187'
;------------------------------------------------------------
...
;------------------------------------------------------------
;channel                   Allocated with name '_radio_set_channel_channel_1_196'
;------------------------------------------------------------
...
_radio_set_channel:
	mov	a,dpl
	mov	dptr,#_radio_set_channel_channel_1_196
	movx	@dptr,a
...
;	radio/radio.c:745: clear_status_registers();
	lcall	_clear_status_registers
;	radio/radio.c:755: register_write(EZRADIOPRO_GPIO0_CONFIGURATION, 0x15);	// RX state (output)
	mov	a,#0x15
	push	acc
	mov	dpl,#0x0B
	lcall	_register_write
	dec	sp
;	radio/radio.c:756: register_write(EZRADIOPRO_GPIO1_CONFIGURATION, 0x12);	// TX state (output)
	mov	a,#0x12
	push	acc
	mov	dpl,#0x0C
	lcall	_register_write
...
;------------------------------------------------------------
;power                     Allocated with name '_radio_set_transmit_power_power_1_220'
;i                         Allocated with name '_radio_set_transmit_power_i_1_221'
;------------------------------------------------------------
...
_radio_set_transmit_power:
	mov	a,dpl
	mov	dptr,#_radio_set_transmit_power_power_1_220
	movx	@dptr,a
;	radio/radio.c:921: for (i=0; i<NUM_POWER_LEVELS; i++) {
	movx	a,@dptr
	mov	r7,a
	mov	r6,#0x00
00106$:
;	radio/radio.c:922: if (power <= power_levels[i]) break;
	mov	a,r6
...
00121$:
	jc	00106$
00113$:
	mov	dptr,#_radio_set_transmit_power_i_1_221
	mov	a,r6
	movx	@dptr,a
;	radio/radio.c:924: if (i == NUM_POWER_LEVELS) {
	cjne	r6,#0x08,00105$
;	radio/radio.c:925: i = NUM_POWER_LEVELS-1;
	mov	dptr,#_radio_set_transmit_power_i_1_221
	mov	a,#0x07
	movx	@dptr,a
00105$:
;	radio/radio.c:927: settings.transmit_power = power_levels[i];
	mov	dptr,#_radio_set_transmit_power_i_1_221
	movx	a,@dptr
	mov	r7,a
	mov	dptr,#_power_levels
	...
	mov	a,r6
	movx	@r0,a
;	radio/radio.c:928: register_write(EZRADIOPRO_TX_POWER, i);
	push	ar7
	mov	dpl,#0x6D
	lcall	_register_write
	dec	sp
...
;------------------------------------------------------------
;id                        Allocated with name '_radio_set_network_id_id_1_226'
;------------------------------------------------------------
...
_radio_set_network_id:
	mov	r7,dph
	mov	a,dpl
	mov	dptr,#_radio_set_network_id_id_1_226
	movx	@dptr,a
	mov	a,r7
	inc	dptr
	movx	@dptr,a
...
_scale_uint32:
	...
	mov	r5,b
	mov	r4,a
	mov	r0,#_scale_uint32_value_1_238
	mov	a,r7
	movx	@r0,a
	inc	r0
	mov	a,r6
...
;------------------------------------------------------------
;status                    Allocated with name '_software_reset_status_1_241'
;------------------------------------------------------------
...
;------------------------------------------------------------
;band                      Allocated with name '_set_frequency_registers_band_1_247'
;carrier                   Allocated with name '_set_frequency_registers_carrier_1_247'
;------------------------------------------------------------
...
_set_frequency_registers:
	...
	mov	r5,b
	mov	r4,a
	mov	r0,#_set_frequency_registers_frequency_1_246
	mov	a,r7
	movx	@r0,a
	inc	r0
	mov	a,r6
...
;	radio/radio.c:1111: frequency -= 480000000UL;
	mov	r0,#(_set_frequency_registers_frequency_1_246 + 1)
	movx	a,@r0
	add	a,#0xC8
	movx	@r0,a
	inc	r0
...
;	radio/radio.c:1112: band  = frequency / 20000000UL;
	...
	lcall	__divulong
	mov	r2,dpl
	mov	dptr,#_set_frequency_registers_band_1_247
	mov	a,r2
	movx	@dptr,a
...
;	-----------------------------------------
_radio_temperature:
;	radio/radio.c:1156: AD0BUSY = 1;		// Start ADC conversion
	setb	_AD0BUSY
;	radio/radio.c:1157: while (AD0BUSY) ;  	// Wait for completion of conversion
00101$:
	jb	_AD0BUSY,00101$
;	radio/radio.c:1159: temp_local = (ADC0H << 8) | ADC0L;
	mov	r7,_ADC0H
	mov	r6,#0x00
	mov	r4,_ADC0L
	mov	r5,#0x00
	...
	mov	a,r5
	orl	ar7,a
;	radio/radio.c:1160: temp_local *= 1.64060;  // convert reading into mV ( (val/1024) * 1680 )  vref=1680mV
	mov	dpl,r6
	mov	dph,r7
	lcall	___sint2fs
	mov	r2,dpl
	...
	mov	r6,dpl
	mov	r7,dph
;	radio/radio.c:1161: temp_local = 25.0 + (temp_local - 1025) / 3.4; // convert mV reading into degC.
	mov	a,r6
	add	a,#0xFF
	mov	r4,a
	mov	a,r7
...
;------------------------------------------------------------
;state                     Allocated with name '_radio_set_diversity_state_1_252'
;------------------------------------------------------------
...
_radio_set_diversity:
	mov	a,dpl
	mov	dptr,#_radio_set_diversity_state_1_252
	movx	@dptr,a
