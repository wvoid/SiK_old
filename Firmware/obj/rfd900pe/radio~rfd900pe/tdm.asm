...
_tdm_show_rssi_sloc5_1_0:
	.ds 2
_sync_tx_windows_old_state_1_171:
	.ds 1
...
_remote_at_cmd:
	.ds 70
_sync_tx_windows_delta_2_178:
	.ds 2
_tdm_serial_loop_len_1_222:
	.ds 1
_tdm_serial_loop_tdelta_1_222:
	.ds 2
_tdm_serial_loop_last_t_1_222:
	.ds 2
_tdm_serial_loop_last_link_update_1_222:
	.ds 2
;--------------------------------------------------------
; external ram data
;--------------------------------------------------------
...
_test_display::
	.ds 1
_link_update_unlock_count_1_201:
	.ds 1
_link_update_temperature_count_1_201:
	.ds 1
_tdm_init_i_1_273:
	.ds 2
_tdm_init_window_width_1_273:
	.ds 4
;--------------------------------------------------------
...
;unlock_count              Allocated with name '_link_update_unlock_count_1_201'
;temperature_count         Allocated with name '_link_update_temperature_count_1_201'
;------------------------------------------------------------
;	radio/tdm.c:383: static uint8_t unlock_count = 10, temperature_count;
	mov	dptr,#_link_update_unlock_count_1_201
	mov	a,#0x0A
	movx	@dptr,a
;--------------------------------------------------------
; Home
...
;------------------------------------------------------------
;old_state                 Allocated with name '_sync_tx_windows_old_state_1_171'
;------------------------------------------------------------
...
;	radio/tdm.c:223: __data enum tdm_state old_state = tdm_state;
	mov	r0,#_tdm_state
	movx	a,@r0
	mov	_sync_tx_windows_old_state_1_171,a
...
;	radio/tdm.c:229: if (old_state == TDM_SILENCE1) {
	mov	a,#0x01
	cjne	a,_sync_tx_windows_old_state_1_171,00106$
...
;	radio/tdm.c:236: } else if (old_state == TDM_RECEIVE || old_state == TDM_SILENCE2) {
	mov	a,#0x02
	cjne	a,_sync_tx_windows_old_state_1_171,00151$
	sjmp	00101$
00151$:
	mov	a,#0x03
	cjne	a,_sync_tx_windows_old_state_1_171,00102$
00101$:
...
;	radio/tdm.c:265: delta = old_remaining - tdm_state_remaining;
	mov	r0,#_tdm_state_remaining
	mov	r1,#_sync_tx_windows_delta_2_178
	setb	c
	movx	a,@r0
	subb	a,r4
	cpl	a
...
;	radio/tdm.c:266: if (old_state != tdm_state ||
	mov	r0,#_tdm_state
	movx	a,@r0
	cjne	a,_sync_tx_windows_old_state_1_171,00113$
;	radio/tdm.c:267: delta > (int16_t)packet_latency/2 ||
	...
	pop	ar2
	pop	ar3
	mov	r0,#_sync_tx_windows_delta_2_178
	clr	c
	movx	a,@r0
	mov	b,a
	mov	a,r6
...
;	radio/tdm.c:269: printf("TDM: %u/%u len=%u ",
	...
	mov	r2,a
	mov	r3,#0x00
	mov	r4,_sync_tx_windows_old_state_1_171
	mov	r5,#0x00
	push	ar7
	push	ar6
	push	ar2
...
;	radio/tdm.c:447: temperature_count++;
	mov	dptr,#_link_update_temperature_count_1_201
	movx	a,@dptr
	add	a,#0x01
	movx	@dptr,a
...
;	radio/tdm.c:516: __pdata uint16_t last_t = timer2_tick();
	lcall	_timer2_tick
	mov	r0,#_tdm_serial_loop_last_t_1_222
	mov	a,dpl
	movx	@r0,a
	inc	r0
	mov	a,dph
...
;	radio/tdm.c:517: __pdata uint16_t last_link_update = last_t;
	mov	r0,#_tdm_serial_loop_last_t_1_222
	mov	r1,#_tdm_serial_loop_last_link_update_1_222
	movx	a,@r0
	movx	@r1,a
	inc	r0
	movx	a,@r0
...
;	radio/tdm.c:552: if (radio_receive_packet(&len, pbuf)) {
	...
	mov	a,#(_pbuf >> 8)
	movx	@r0,a
	mov	dptr,#_tdm_serial_loop_len_1_222
	mov	b,#0x60
	push	ar3
	push	ar2
...
;	radio/tdm.c:566: if (len < 2) {
	mov	r0,#_tdm_serial_loop_len_1_222
	movx	a,@r0
	cjne	a,#0x02,00397$
00397$:
	jnc	00398$
...
;	radio/tdm.c:634: tdelta = tnow - last_t;
	mov	r0,#_tdm_serial_loop_last_t_1_222
	mov	r1,#_tdm_serial_loop_tdelta_1_222
	setb	c
	movx	a,@r0
	subb	a,r2
	cpl	a
...
;	radio/tdm.c:635: tdm_state_update(tdelta);
	mov	r0,#_tdm_serial_loop_tdelta_1_222
	movx	a,@r0
	mov	dpl,a
	inc	r0
	movx	a,@r0
...
;	radio/tdm.c:639: if (tnow - last_link_update > 32768) {
	mov	r0,#_tdm_serial_loop_last_link_update_1_222
	setb	c
	movx	a,@r0
	subb	a,r2
	cpl	a
...
;i                         Allocated with name '_tdm_init_i_1_273'
;air_rate                  Allocated with name '_tdm_init_air_rate_1_273'
;window_width              Allocated with name '_tdm_init_window_width_1_273'
;------------------------------------------------------------
...
;	radio/tdm.c:1020: window_width = 3*(packet_latency+(max_data_packet_length*(uint32_t)ticks_per_byte));
	...
	mov	r6,b
	mov	r7,a
	mov	dptr,#_tdm_init_window_width_1_273
	mov	a,r4
	movx	@dptr,a
	mov	a,r5
	inc	dptr
